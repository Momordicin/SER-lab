...
# ============================================================
# filename parsing / dataset split / SERDataset /
# DataCollator / WeightedTrainer / helpers unchangeded from ser_wav2vec2.py，core changes are in build_model and train_ser 
# ============================================================
...
def split_dataset(
    source_dir, target_dir, train_ratio=0.7, val_ratio=0.15,
    test_ratio=0.15, seed=42, make_metadata_csv=True,
):
    ...
    for lab, paths in sorted(by_label.items()):
        paths = sorted(paths); rnd.shuffle(paths); n = len(paths)
        if n == 1: train_files += paths; continue
        if n == 2:
        ...
        if n_test < 0:
            deficit = -n_test; take = min(deficit, n_val); n_val -= take; n_test += take
        if n_test < 0:
            deficit = -n_test; take = min(deficit, max(0, n_train - 1)); n_train -= take; n_test += take
        if n_train < 1:
            n_train = 1
            if n_val > 0: n_val -= 1
            n_test = n - n_train - n_val
    ...
    def copy_into(split_name, paths):
        for p in paths:
            lab = extract_label_from_name(p)
    ...
    copy_into("train", train_files); copy_into("val", val_files); copy_into("test", test_files)
...
class SERDataset(torch.utils.data.Dataset):
    def __init__(self, items, processor, target_sr=16000, max_seconds=6.0, min_seconds=0.2, trim_or_pad=True):
        self.items = items; self.processor = processor
    ...
    def _load_wav(self, path):
        ...
        audio = np.mean(audio, axis=1, dtype=np.float32) if audio.shape[1] > 1 else audio[:, 0]
        if sr != self.target_sr:
            audio = torchaudio.transforms.Resample(sr, self.target_sr)(torch.from_numpy(audio)).numpy()
        if self.trim_or_pad:
            if audio.shape[0] > self.max_len: audio = audio[:self.max_len]
    ...
    def __getitem__(self, idx):
        ...
        return {"input_values": self._load_wav(path), "labels": int(label_id), "path": path}
...
class WeightedTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss = torch.nn.CrossEntropyLoss(weight=cw)(
...
def resolve_label_name(id2label, idx: int):
    ...
    try: return id2label[idx]
    except Exception: return f"label_{idx}"

def get_label_names_from_train(train_dir):
    labs = []
...
def load_split_items(split_dir, label2id):
    items = []
...
def compute_class_weights(train_items, num_labels):
...
def make_training_args(out_dir, batch_size=8, epochs=10, lr=3e-5,
                       weight_decay=0.01, logging_steps=50, num_workers=0, seed=42):
    return TrainingArguments(
        ...
        report_to="none",
        seed=seed,
        # ↓↓↓ CHANGE 2: WavLM-Large + gradient_checkpointing may require more VRAM, 
        # gradient_checkpointing=True,   # can reduce VRAM usage by 40%
    )


# ============================================================
# CHANGE 3: build_model —— core change
# ============================================================
def build_model(
    model_name: str,
    num_labels: int,
    ...
    model = WavLMForSequenceClassification.from_pretrained(  # ← WavLM
        model_name,
    ...
    if freeze_feature_extractor:
        # WavLM CNN feature extractor: model.wavlm.feature_extractor
        for p in model.wavlm.feature_extractor.parameters():  # ← wavlm
...
def train_ser(
    data_root,
    # ↓↓↓ CHANGE 4: replace with wavlm-base-plus
    ...
    model = build_model(model_name, len(labels), id2label, label2id, freeze_feature_extractor=True)
...
# ============================================================
# CHANGE 5: evaluate_on_test / predict_wav
# ============================================================
def evaluate_on_test(
    data_root, ckpt_dir, sr_target=16000, max_seconds=6.0,
    batch_size=8, preds_csv="./wavlm_preds.csv", report_txt=None,
):
    train_dir = os.path.join(data_root, "train")
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            y_true.append(labels_t); y_pred.append(np.argmax(logits, axis=-1))
    ...
    print("\nTEST RESULTS")
    print("Accuracy:", acc); print("F1 (macro):", f1m); print("F1 (weighted):", f1w)
    present_ids   = sorted(set(y_true) | set(y_pred))
    present_names = [id2label[i] for i in present_ids]
    print("\nPer-class report:\n",
          classification_report(y_true, y_pred, labels=present_ids, target_names=present_names))
    print("Confusion matrix:\n", confusion_matrix(y_true, y_pred, labels=present_ids))

    if pd is None:
        np.savetxt(preds_csv, np.vstack([y_true, y_pred]).T, fmt="%d",
    ...
    if report_txt:
        rep = classification_report(y_true, y_pred, labels=present_ids, target_names=present_names)
        with open(report_txt, "w") as f:
            f.write("TEST RESULTS\n")
            f.write(f"Accuracy: {acc:.4f}\nF1 (macro): {f1m:.4f}\nF1 (weighted): {f1w:.4f}\n\n")
            f.write(rep)
        print(f"Wrote detailed report to {report_txt}")
...
def predict_wav(wav_path, ckpt_dir, sr_target=16000):
    ...
    audio = np.mean(audio, axis=1).astype(np.float32) if audio.shape[1] > 1 else audio[:, 0]
    if sr != sr_target:
    ...
    with torch.no_grad():
        ...
        probs = torch.softmax(model(**batch).logits, dim=-1).cpu().numpy()[0]

    id2label = model.config.id2label
    pred_id = int(np.argmax(probs))
    return {
        "label": resolve_label_name(id2label, pred_id),
        "probs": {resolve_label_name(id2label, i): float(probs[i]) for i in range(len(probs))}
    }


def predict_folder(folder, ckpt_dir, out_json="predictions.json", sr_target=16000):
    results = []
...
def main():
    parser = argparse.ArgumentParser(description="WavLM SER (emotion from filename tag)")
    ...
    p_tr.add_argument("--out_dir",     default="./ser_wavlm_ckpt")
    ...
    p_te.add_argument("--ckpt_dir",    default="./ser_wavlm_ckpt")
    ...
    p_te.add_argument("--preds_csv",   default="./wavlm_preds.csv")
    p_te.add_argument("--report_txt",  default=None)

    p_pw = sub.add_parser("predict_wav")
    p_pw.add_argument("--wav_path",  required=True)
    p_pw.add_argument("--ckpt_dir",  default="./ser_wavlm_ckpt")
    p_pw.add_argument("--sr_target", type=int, default=16000)

    p_pf = sub.add_parser("predict_folder")
    p_pf.add_argument("--folder",    required=True)
    p_pf.add_argument("--ckpt_dir",  default="./ser_wavlm_ckpt")
    ...
    if args.cmd == "test":
        evaluate_on_test(args.data_root, args.ckpt_dir, args.sr_target,
                         ...
                         args.preds_csv, args.report_txt); return
    if args.cmd == "predict_wav":
        print(json.dumps(predict_wav(args.wav_path, args.ckpt_dir, args.sr_target), indent=2)); return
    if args.cmd == "predict_folder":
        predict_folder(args.folder, args.ckpt_dir, args.out_json, args.sr_target); return
